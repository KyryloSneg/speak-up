...
async function sendSDPEventHandlerCb(
  io: IO,
  socket: IOSocket<true>,
  unvalidatedData: unknown,
): Promise<void> {
  function emitErrorEvent(error: string): void {
    socket.emit(SocketResponseEvents.SEND_SDP, {
      error,
    });
  }

  const validationResult =
    getZodSendSDPDataValidation().safeParse(unvalidatedData);
  ...
  recipientSocket.emit(SocketEvents.RECEIVED_SDP, {
...
const sendSDPEventHandler = catchEventHandlerErrorDecorator(
  sendSDPEventHandlerCb,
  SocketResponseEvents.SEND_SDP,
);

export default sendSDPEventHandler;
